...
def render() -> None:
    ...
    st.title(":material/account_balance_wallet: Budget & Goals")
    st.caption(
        f"Your budget planner — currency: **{currency_code}** ({get_country_name(country)}). "
        "All figures are monthly unless noted."
    )
...
def _render_budget_tab(sym: str, currency_code: str) -> None:
    ...
    income = st.number_input(
        f"Monthly take-home income ({sym})",
        min_value=0.0,
    ...
    if income <= 0:
        st.info("Enter your monthly take-home income above to see your budget breakdown.", icon=":material/info:")
        return

    st.divider()
    st.subheader("50/30/20 framework")
    st.caption("Adjust the percentages to fit your reality. The total must equal 100%.")

    col1, col2, col3 = st.columns(3)
    with col1:
        needs_pct = st.number_input("Needs (%)", min_value=0, max_value=100, value=50, step=1)
    with col2:
        wants_pct = st.number_input("Wants (%)", min_value=0, max_value=100, value=30, step=1)
    with col3:
        savings_pct = st.number_input("Savings / Debt (%)", min_value=0, max_value=100, value=20, step=1)

    total_pct = needs_pct + wants_pct + savings_pct
    if total_pct != 100:
        st.warning(f"Percentages total {total_pct}% — adjust to reach exactly 100%.", icon=":material/warning:")
    ...
    col_a.metric("Needs", f"{sym}{needs_amt:,.2f}", f"{needs_pct}% of income")
    col_b.metric("Wants", f"{sym}{wants_amt:,.2f}", f"{wants_pct}% of income")
    col_c.metric("Savings / Debt", f"{sym}{savings_amt:,.2f}", f"{savings_pct}% of income")
    ...
    chart = (
        ...
        .encode(
            ...
            y=alt.Y("Amount:Q", title=f"Amount ({currency_code})"),
    ...
    st.caption("Log your actual monthly expenses to compare against your plan.")
    ...
    with st.form("add_expense", clear_on_submit=True):
        ...
        with fc1:
            item_name = st.text_input("Description", placeholder="e.g., Rent")
        with fc2:
            item_amount = st.number_input(f"Amount ({sym})", min_value=0.0, step=10.0, format="%.2f")
        with fc3:
            item_type = st.selectbox("Type", ["Need", "Want", "Saving / Debt"])
        with fc4:
            submitted = st.form_submit_button(":material/add: Add", type="primary")
    ...
    if budget_items:
        ...
        col_tot.metric("Total expenses", f"{sym}{total_expenses:,.2f}")
        col_rem.metric(
        ...
        if st.button(":material/delete: Clear all expenses", type="secondary"):
            st.session_state.budget_items = []
            st.rerun()
    else:
        st.info("No expenses logged yet. Add your first expense above.", icon=":material/info:")
...
def _render_goals_tab(sym: str, currency_code: str) -> None:
    ...
    # Add new goal
    with st.expander(":material/add_circle: Add a new goal", expanded=len(budget_goals) == 0):
        with st.form("add_goal", clear_on_submit=True):
            gc1, gc2, gc3 = st.columns([3, 2, 2])
            with gc1:
                # Combine preset goal names + custom goals
                ...
                goal_choice = st.selectbox("Goal type", all_names)
            with gc2:
                goal_target = st.number_input(
                    f"Target ({sym})", min_value=0.0, step=100.0, format="%.2f"
                )
            with gc3:
                goal_saved = st.number_input(
                    f"Already saved ({sym})", min_value=0.0, step=100.0, format="%.2f"
                )

            if goal_choice == "Custom…":
                goal_label = st.text_input("Custom goal name")
            else:
                goal_label = goal_choice

            goal_monthly = st.number_input(
                f"Monthly contribution ({sym})", min_value=0.0, step=50.0, format="%.2f"
            )
            add_goal = st.form_submit_button(":material/savings: Add goal", type="primary")
    ...
    for idx, goal in enumerate(budget_goals):
        with st.container(border=True):
            ...
            mc1.metric("Target", f"{sym}{goal['target']:,.0f}")
            mc2.metric("Saved", f"{sym}{goal['saved']:,.0f}")
            mc3.metric("Remaining", f"{sym}{remaining:,.0f}")
            if months_left is not None:
                mc4.metric(
                    "Est. months left",
                    f"{months_left}",
                    help=f"At {sym}{goal.get('monthly',0):,.0f}/month",
                )
            else:
                mc4.caption("Set a monthly contribution to estimate completion date.")

            # Update saved amount inline
            new_saved = st.number_input(
                f"Update saved amount ({sym})",
                min_value=0.0,
    ...
    st.markdown(f"**Overall progress across all goals:** {overall_pct:.1f}%")
    st.markdown(_pct_bar(overall_pct), unsafe_allow_html=True)
    st.caption(f"{sym}{total_saved:,.0f} saved toward {sym}{total_target:,.0f} total")
